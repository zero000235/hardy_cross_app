...
with st.sidebar:
    ...
    st.markdown("---")
    max_iter = st.number_input("최대 반복 횟수 (Iterations)", min_value=10, max_value=1000, value=100, step=10)
    ...
    if pdp is not None:
        fluid_list = list(pdp.FLUID_OPTIONS.keys())
        fluid_display = [pdp.FLUID_OPTIONS[f] for f in fluid_list]
        ...
        material_list = list(pdp.ROUGHNESS.keys())
        def_idx = 2 if len(material_list) > 2 else 0
        pipe_material = st.selectbox("배관 재질 (조도 ε 결정)", material_list, index=def_idx)
    else:
        st.warning("⚠️ 물성치 모듈(pressure_drop.py)이 없어 기본값(물 20°C, 상업용 강관)이 적용됩니다.")
        fluid_type = "Water"
        fluid_temp = 20.0
        pipe_material = "Commercial Steel (상업용 강관)"
...
if calc_btn:
    try:
        ...
        if pdp is not None:
            try: 
                rho, mu, _ = pdp.get_fluid_properties(fluid_type, fluid_temp)
                epsilon = pdp.ROUGHNESS.get(pipe_material, 4.6e-5)
            except: pass
                ...
                for p in p_list:
                    ...
                    if n_exp == 2.0: # Darcy-Weisbach
                        if abs_Q < 1e-6:
                            ...
                            Re_val = (rho * v * D_m) / mu
                            if pdp is not None:
                                f_val, _ = pdp.calc_friction_factor(Re_val, D_m, epsilon)
                            else:
                                f_val = 0.02
